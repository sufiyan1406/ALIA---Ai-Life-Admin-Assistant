-- ============================================================
-- MIGRATION 005: Phase 4 — Smart Reminder & Automation Engine
-- Run this in Supabase SQL Editor after 004_mvp_file_gamification.sql
-- ============================================================
...
WHERE sent = true AND status = 'pending';

-- 3. Performance index: scheduler only scans pending reminders by due time
CREATE INDEX IF NOT EXISTS idx_reminders_pending_due
  ON reminders (status, remind_at) WHERE status = 'pending';

-- 4. Add user reminder preference columns
